...
vim:
	...
	cd $(BUILD_DIR)/vim/bundle
	for r in $(REPOS); do dir=`echo $$r | perl -pe 's/.*\/([^\/]*).git$$/\1/'`; if [ -d $$dir ]; then cd $$dir; git pull; cd ..; else git clone $$r; fi done || [ $$? -eq 1 ]
...
zsh: variables build/shell/zshrc.txt
